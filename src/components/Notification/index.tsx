...
const Notification: React.FC<NotificationType> = ({ children }) => {
    ...
    if (notificationState?.type === NotificationTypeValue.SUCCESS) {
        return (
            <>
                {notificationShow &&
                    <div className="fixed left-0 top-0 flex justify-center items-center bg-[#863ddb40] w-full min-h-screen">
                        <div ref={rootRef} className="bg-white rounded-[25px] w-full md:w-[384px] max-w-[calc(100%-20px)] pt-8 pb-10">
                            <div className="text-2xl font-bold text-center text-[#0FBC00] mb-4">{notificationState?.text}</div>
    ...
    if (notificationState?.type === NotificationTypeValue.ERROR) {
        return (
            <>
                {notificationShow &&
                    <div className="fixed left-0 top-0 flex justify-center items-center bg-[#863ddb40] w-full min-h-screen">
